...
export function ActivityItem({ activity }: ActivityItemProps) {
    ...
    return (
        <div className={`cursor-pointer px-4 py-3 bg-zinc-900 border border-zinc-800 rounded-xl shadow-sm flex items-center gap-3 transition-all hover:bg-zinc-800 ${isDone ? 'opacity-60' : 'opacity-100'}`} onClick={toggleComplete}>
            <button
                disabled={loading}
